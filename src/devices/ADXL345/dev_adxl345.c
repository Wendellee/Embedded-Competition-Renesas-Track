...
static uint8_t adxl345_get_register_value(SPIDevice *pADXLSPI, uint8_t register_address);
/*! Writes data into a register. */
static void adxl345_set_register_value(SPIDevice *pADXLSPI,uint8_t register_address, uint8_t register_value);
/*! Burst reads from a ADXL345 Internal Register. */
static int adxl345_get_burst_register_value(SPIDevice *pADXLSPI, unsigned char addr);
/*! Read all axes. */
static void adxl345_get_all_axes(SPIDevice *pADXLSPI);
/*! Places the device into standby/measure mode. */
static void adxl345_set_power_mode(SPIDevice *pADXLSPI, uint8_t pwr_mode);
/*! Reads the raw output data of each axis. */
static void adxl345_get_xyz(SPIDevice *pADXLSPI, int16_t *x, int16_t *y, int16_t *z);
/*! Reads the raw output data of each axis and converts it to g. */
static void adxl345_get_g_xyz(SPIDevice *pADXLSPI, float *x, float *y, float *z);
/*! Enables/disables the tap detection. */
static void adxl345_set_tap_detection(SPIDevice *pADXLSPI, uint8_t tap_type,
                                      uint8_t tap_axes,
                                      uint8_t tap_dur,
                                      uint8_t tap_latent,
                                      uint8_t tap_window,
...
static void adxl345_set_activity_detection(SPIDevice *pADXLSPI, uint8_t act_on_off,
                                           uint8_t act_axes,
                                           uint8_t act_ac_dc,
                                           uint8_t act_thresh,
                                           uint8_t act_int);
/*! Enables/disables the inactivity detection. */
static void adxl345_set_inactivity_detection(SPIDevice *pADXLSPI, uint8_t inact_on_off,
                                             uint8_t inact_axes,
                                             uint8_t inact_ac_dc,
...
static void adxl345_set_free_fall_detection(SPIDevice *pADXLSPI, uint8_t ff_on_off,
                                            uint8_t ff_thresh,
                                            uint8_t ff_time,
                                            uint8_t ff_int);
/*! Sets an offset value for each axis (Offset Calibration). */
static void adxl345_set_offset(SPIDevice *pADXLSPI, uint8_t x_offset,uint8_t y_offset,uint8_t z_offset);
/*! Selects the measurement range. */
static void adxl345_set_range_resolution(SPIDevice *pADXLSPI, uint8_t g_range,uint8_t full_res);

static int ADXLDevInit_0(struct ADXL345Dev *ptdev);
static int ADXLDevRead_0(SPIDevice *pADXLSPI, struct ADXL345Dev *ptdev);
static int ADXLDevStart_0(SPIDevice *pADXLSPI, struct ADXL345Dev *ptdev);
static int ADXLDevStop_0(SPIDevice *pADXLSPI, struct ADXL345Dev *ptdev);

static int ADXLDevInit_1(struct ADXL345Dev *ptdev);
static int ADXLDevRead_1(SPIDevice *pADXLSPI, struct ADXL345Dev *ptdev);
static int ADXLDevStart_1(SPIDevice *pADXLSPI, struct ADXL345Dev *ptdev);
static int ADXLDevStop_1(SPIDevice *pADXLSPI, struct ADXL345Dev *ptdev);

static IODevice  *pINT1IO_0 = NULL;
static IODevice  *pINT1IO_1 = NULL;
static SPIDevice *pADXLSPI_0 = NULL;
static SPIDevice *pADXLSPI_1 = NULL;


static ADXL345Device gADXL345_0 = {
    .name = "ADXL345_0",
    .selected_range = 2,
    .full_resolution_set = 0,
    .Init   = ADXLDevInit_0,
    .Read   = ADXLDevRead_0,
    .Start  = ADXLDevStart_0,
    .Stop   = ADXLDevStop_0
};

static ADXL345Device gADXL345_1 = {
    .name = "ADXL345_1",
    .selected_range = 2,
    .full_resolution_set = 0,
    .Init   = ADXLDevInit_1,
    .Read   = ADXLDevRead_1,
    .Start  = ADXLDevStart_1,
    .Stop   = ADXLDevStop_1
};
...
static int ADXLDevInit_0(struct ADXL345Dev *ptdev)
{
    ...
    pINT1IO_0 = IODeviceFind("ADXL345_0 INT1");
    if(NULL == pINT1IO_0)    return -ENXIO;

    pADXLSPI_0 = SPIDeviceFind("ADXL345_0 SPI");
    if(NULL == pADXLSPI_0)    return -ENXIO;
    if(ESUCCESS != pADXLSPI_0->Init(pADXLSPI_0))    return -EIO;

    if (adxl345_get_register_value(pADXLSPI_0,ADXL345_DEVID) != ADXL345_ID)
    {
        Serial_Printf(g_debug_uart4.p_ctrl,"Failed to read ADXL345_0's ID!\r\n");
        return -EIO;
    }

    adxl345_set_power_mode(pADXLSPI_0,0x01);   /* measure(1)/standby(0) mode.*/
    adxl345_set_range_resolution(pADXLSPI_0,ADXL345_RANGE_PM_4G,  /* Range option. +- 16g */
                                 ADXL345_FULL_RES);     /*Enables full resolution*/
    adxl345_set_tap_detection(pADXLSPI_0,ADXL345_SINGLE_TAP | ADXL345_DOUBLE_TAP,    /* Tap type (none, single, double) */
    ...
    adxl345_set_activity_detection(pADXLSPI_0,0x01,    /* enables(1)/disable(0) the activity detection. */
    ...
    adxl345_set_free_fall_detection(pADXLSPI_0,0x01,   /* disables(0)/enable(1) the free-fall detection. */
                                    0x08,   /* Threshold value for free-fall detection. The scale factor */
                                    0x0A,   /* Time value for free-fall detection */
                                    0x00);  /* Interrupts pin. */
	adxl345_get_all_axes(pADXLSPI_0);
...
static int ADXLDevInit_1(struct ADXL345Dev *ptdev)
{
    ...
    pINT1IO_1 = IODeviceFind("ADXL345_1 INT1");
    if(NULL == pINT1IO_1)    return -ENXIO;

    pADXLSPI_1 = SPIDeviceFind("ADXL345_1 SPI");
    if(NULL == pADXLSPI_1)    return -ENXIO;
    if(ESUCCESS != pADXLSPI_1->Init(pADXLSPI_1))    return -EIO;

    if (adxl345_get_register_value(pADXLSPI_1,ADXL345_DEVID) != ADXL345_ID)
    {
        Serial_Printf(g_debug_uart4.p_ctrl,"Failed to read ADXL345_1's ID!\r\n");
        return -EIO;
    }

    adxl345_set_power_mode(pADXLSPI_1,0x01);   /* measure(1)/standby(0) mode.*/
    adxl345_set_range_resolution(pADXLSPI_1,ADXL345_RANGE_PM_4G,  /* Range option. +- 16g */
                                 ADXL345_FULL_RES);     /*Enables full resolution*/
    adxl345_set_tap_detection(pADXLSPI_1,ADXL345_SINGLE_TAP | ADXL345_DOUBLE_TAP,    /* Tap type (none, single, double) */
                              ADXL345_TAP_X_EN | ADXL345_TAP_Y_EN | ADXL345_TAP_Z_EN, /* tap_axes */
                              0x0D,   /* tap_dur */
                              0x50,   /* tap_latent */
                              0xF0,   /* tap_window */
                              0x20,   /* Tap threshold */
                              0x00);    /* Interrupts pin.0x0 - interrupts on INT1 pin */
    adxl345_set_activity_detection(pADXLSPI_1,0x01,    /* enables(1)/disable(0) the activity detection. */
                                   ADXL345_ACT_X_EN | ADXL345_ACT_Y_EN | ADXL345_ACT_Z_EN,  /* Axes which participate in detecting activity. */
                                   0x00,    /* dc(0)/ac(ADXL345_ACT_ACDC)-coupled operation. */
                                   0x08,    /* Threshold value for detecting activity */
                                   0x00);   /* Interrupts pin.0-int1 */
    adxl345_set_free_fall_detection(pADXLSPI_1,0x01,   /* disables(0)/enable(1) the free-fall detection. */
                                    0x08,   /* Threshold value for free-fall detection. The scale factor */
                                    0x0A,   /* Time value for free-fall detection */
                                    0x00);  /* Interrupts pin. */
	adxl345_get_all_axes(pADXLSPI_1);
...
static int ADXLDevRead_0(SPIDevice *pADXLSPI, struct ADXL345Dev *ptdev)
{
    ...
    adxl345_get_g_xyz(pADXLSPI,&ptdev->value.x, &ptdev->value.y, &ptdev->value.z);
    return ESUCCESS;
}
...
{
    if(NULL == ptdev)       return -EINVAL;
    adxl345_set_register_value(pADXLSPI, ADXL345_POWER_CTL, ADXL345_PCTL_MEASURE);
    return ESUCCESS;
}
...
{
    if(NULL == ptdev)       return -EINVAL;
    adxl345_set_register_value(pADXLSPI, ADXL345_POWER_CTL, ADXL345_PCTL_SLEEP);
    return ESUCCESS;
}
 ...
 *
 * @param pADXLSPI - Pointer to the ADXL345 SPI device.
...
static void adxl345_get_all_axes(SPIDevice *pADXLSPI)
{
	adxl345_get_burst_register_value(pADXLSPI, ADXL345_DATAX0);
	adxl345_get_burst_register_value(pADXLSPI, ADXL345_DATAY0);
	adxl345_get_burst_register_value(pADXLSPI, ADXL345_DATAZ0);
}
...
static void adxl345_set_power_mode(SPIDevice *pADXLSPI, uint8_t pwr_mode)
{
    ...
    old_power_ctl = adxl345_get_register_value(pADXLSPI,ADXL345_POWER_CTL);
    new_power_ctl = old_power_ctl & ~ADXL345_PCTL_MEASURE;
    new_power_ctl = new_power_ctl | (pwr_mode * ADXL345_PCTL_MEASURE);
    adxl345_set_register_value(pADXLSPI,ADXL345_POWER_CTL, new_power_ctl);
}
...
static void adxl345_get_g_xyz(SPIDevice *pADXLSPI, float *x, float *y, float *z)
{
    ...
    adxl345_get_xyz(pADXLSPI, &x_data, &y_data, &z_data);
    if(pADXLSPI == pADXLSPI_0)
    {
        *x = (float)(gADXL345_0.full_resolution_set  ? (x_data * ADXL345_SCALE_FACTOR) :
                    (x_data * ADXL345_SCALE_FACTOR * (gADXL345_0.selected_range >> 1)));
        *y = (float)(gADXL345_0.full_resolution_set  ? (y_data * ADXL345_SCALE_FACTOR) :
                    (y_data * ADXL345_SCALE_FACTOR * (gADXL345_0.selected_range >> 1)));   
        *z = (float)(gADXL345_0.full_resolution_set  ? (z_data * ADXL345_SCALE_FACTOR) :       
                    (z_data * ADXL345_SCALE_FACTOR * (gADXL345_0.selected_range >> 1)));
    }
    else if(pADXLSPI == pADXLSPI_1)
    {
        *x = (float)(gADXL345_1.full_resolution_set  ? (x_data * ADXL345_SCALE_FACTOR) :
                    (x_data * ADXL345_SCALE_FACTOR * (gADXL345_1.selected_range >> 1)));
        *y = (float)(gADXL345_1.full_resolution_set  ? (y_data * ADXL345_SCALE_FACTOR) :
                    (y_data * ADXL345_SCALE_FACTOR * (gADXL345_1.selected_range >> 1)));   
        *z = (float)(gADXL345_1.full_resolution_set  ? (z_data * ADXL345_SCALE_FACTOR) :   
                    (z_data * ADXL345_SCALE_FACTOR * (gADXL345_1.selected_range >> 1)));
    }
    else{
        return;
    }
                                      ...
                                      uint8_t tap_int)
{
    ...
    old_tap_axes = adxl345_get_register_value(pADXLSPI, ADXL345_TAP_AXES);
    new_tap_axes = old_tap_axes & ~(ADXL345_TAP_X_EN |ADXL345_TAP_Y_EN |ADXL345_TAP_Z_EN);
    new_tap_axes = new_tap_axes | tap_axes;
    adxl345_set_register_value(pADXLSPI, ADXL345_TAP_AXES, new_tap_axes);
    adxl345_set_register_value(pADXLSPI, ADXL345_DUR, tap_dur);
    adxl345_set_register_value(pADXLSPI, ADXL345_LATENT, tap_latent);
    adxl345_set_register_value(pADXLSPI, ADXL345_WINDOW, tap_window);
    adxl345_set_register_value(pADXLSPI, ADXL345_THRESH_TAP, tap_thresh);
    old_int_map = adxl345_get_register_value(pADXLSPI, ADXL345_INT_MAP);
    new_int_map = old_int_map & (~(ADXL345_SINGLE_TAP | ADXL345_DOUBLE_TAP));
    new_int_map = new_int_map | tap_int;
    adxl345_set_register_value(pADXLSPI, ADXL345_INT_MAP, new_int_map);
    old_int_enable = adxl345_get_register_value(pADXLSPI, ADXL345_INT_ENABLE);
    new_int_enable = old_int_enable & (~(ADXL345_SINGLE_TAP | ADXL345_DOUBLE_TAP));
    new_int_enable = new_int_enable | tap_type;
    adxl345_set_register_value(pADXLSPI, ADXL345_INT_ENABLE, new_int_enable);
}
                                           ...
                                           uint8_t act_int)
{
    ...
    old_act_inact_ctl = adxl345_get_register_value(pADXLSPI, ADXL345_ACT_INACT_CTL);
    ...
    adxl345_set_register_value(pADXLSPI, ADXL345_ACT_INACT_CTL, new_act_inact_ctl);
    adxl345_set_register_value(pADXLSPI, ADXL345_THRESH_ACT, act_thresh);
    old_int_map = adxl345_get_register_value(pADXLSPI, ADXL345_INT_MAP);
                                             ...
                                             uint8_t inact_int)
{
    ...
    adxl345_set_register_value(pADXLSPI, ADXL345_ACT_INACT_CTL, new_act_inact_ctl);
    adxl345_set_register_value(pADXLSPI, ADXL345_THRESH_INACT, inact_thresh);
    adxl345_set_register_value(pADXLSPI, ADXL345_TIME_INACT, inact_time);
    old_int_map = adxl345_get_register_value(pADXLSPI, ADXL345_INT_MAP);
                                            ...
                                            uint8_t ff_int)
{
    ...
    adxl345_set_register_value(pADXLSPI, ADXL345_THRESH_FF, ff_thresh);
    adxl345_set_register_value(pADXLSPI, ADXL345_TIME_FF, ff_time);
    old_int_map = adxl345_get_register_value(pADXLSPI, ADXL345_INT_MAP);
...
static void adxl345_set_offset(SPIDevice *pADXLSPI, uint8_t x_offset,uint8_t y_offset,uint8_t z_offset)
{
    ...
    adxl345_set_register_value(pADXLSPI, ADXL345_OFSX, x_offset);
    adxl345_set_register_value(pADXLSPI, ADXL345_OFSY, y_offset);
    adxl345_set_register_value(pADXLSPI, ADXL345_OFSZ, z_offset);
}
...
static void adxl345_set_range_resolution(SPIDevice *pADXLSPI, uint8_t g_range,uint8_t full_res)
{
    ...
    if(pADXLSPI == pADXLSPI_0)    
    {
        old_data_format = adxl345_get_register_value(pADXLSPI, ADXL345_DATA_FORMAT);
        ...
        adxl345_set_register_value(pADXLSPI, ADXL345_DATA_FORMAT, new_data_format);
        gADXL345_0.selected_range = (uint8_t)(1 << (g_range + 1));
        gADXL345_0.full_resolution_set = full_res ? 1 : 0;
    }
    else if(pADXLSPI == pADXLSPI_1)
    {
        old_data_format = adxl345_get_register_value(pADXLSPI, ADXL345_DATA_FORMAT);
        ...
        new_data_format =  new_data_format | ADXL345_RANGE(g_range) | full_res;
        adxl345_set_register_value(pADXLSPI, ADXL345_DATA_FORMAT, new_data_format);
        gADXL345_1.selected_range = (uint8_t)(1 << (g_range + 1));
        gADXL345_1.full_resolution_set = full_res ? 1 : 0;
    }
    else{
        return;
    }
